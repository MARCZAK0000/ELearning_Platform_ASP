...
namespace ELearning_Platform.Infrastructure.Repository
{
        ...
        UserManager<Account> userManager) : ISchoolRepository
    {
        ...
        #region Query Method
        public async Task<Pagination<Lesson>> FindLessonBySubjectIDAsync(string subjectID, PaginationModelDto paginationModelDto, CancellationToken cancellationToken)
        {
            var paginationBuilder = new PaginationBuilder<Lesson>();

            var lessonBase = _platformDb
                .Lessons
                .Include(x => x.LessonMaterials)
                .Where(pr => pr.SubjectID == subjectID);

            var count = await lessonBase.CountAsync(cancellationToken: cancellationToken);

            //var columnSelector = new Dictionary<OrderByEnum, Expression<Func<Lesson, object>>>
            //{
